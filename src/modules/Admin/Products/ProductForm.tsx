...
const ProductForm: React.FC<ProductFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const saveVariant = async (values: VariantDraft) => {
    ...
    try {
      ...
      if (variantEditing) {
        ...
        // Load lại dữ liệu từ variant vừa tạo vào form (nhưng để trống variant_value)
        variantForm.setFieldsValue({
          variant_type: values.variant_type,
          variant_value: '', // Để trống giá trị để user nhập mới
          price_adjustment: values.price_adjustment ?? 0,
          stock_quantity: values.stock_quantity ?? 0,
        });
  ...
  const duplicateVariant = (variant: ProductVariant) => {
    ...
    variantForm.setFieldsValue({
      ...
      variant_value: '', // Để trống để user nhập mới
        ...
        if (urls && urls.length > 0) {
          return (
            <Badge count={urls.length} offset={[-5, 5]}>
              <Image
                src={urls[0]}
                alt={`${record.variant_type}: ${record.variant_value}`}
                width={60}
                height={60}
